...
- Professors/tutors can create new sessions by entering subject, schedule, modality, and available spots.
- Access is restricted based on the user’s role.
- Users are redirected to their specific dashboard right after logging in.
...
- Roles were assigned to each team member, and the Scrum Master role rotates weekly.
- A first planning meeting was held to build the initial backlog.
- Sprint 1 was configured with selected tasks from the backlog.
- UI mockups are being developed to outline how each screen will work.
- Documentation is being maintained in both the README and the Wiki.
...
3. Run the module that handles the DNI login flow.
4. Test the functionalities described in the user stories:
   - Tutoring search
